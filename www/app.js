...
// ===========================
// CORPORATE FINANCE ADVISOR CHAT
// ===========================

function handleFinanceChatKeyPress(event) {
    if (event.key === 'Enter' && !event.shiftKey) {
        event.preventDefault();
...
function addFinanceChatMessage(message, sender) {
    const messagesContainer = document.getElementById('finance-chat-messages');
...
function showFinanceTyping() {
    ...
    if (!messagesContainer) return;
    const typingDiv = document.createElement('div');
    typingDiv.className = 'message emma finance-typing';
    typingDiv.innerHTML = `<div class="message-bubble">Thinking…</div>`;
    messagesContainer.appendChild(typingDiv);
    messagesContainer.scrollTop = messagesContainer.scrollHeight;
}

function hideFinanceTyping() {
    const typing = document.querySelector('.finance-typing');
...
async function sendFinanceChatMessage() {
    ...
    addFinanceChatMessage(message, 'user');
    input.value = '';
    showFinanceTyping();

    try {
        const userFinanceContext = {
            monthlyIncome: Number(localStorage.getItem('ontop_income') || AppState.finances?.income || 0),
            fixedBills: (AppState.finances?.bills || []).map(b => ({ name: b.name, amount: b.amount })),
            expenses: (AppState.finances?.expenses || []).map(e => ({ name: e.name, amount: e.amount })),
            savings: Number(localStorage.getItem('ontop_savings_balance') || 0),
            goals: (AppState.finances?.goals || []).map(g => ({ title: g.title, target: g.target, current: g.current })),
            riskTolerance: localStorage.getItem('ontop_risk') || 'balanced',
            timeHorizonYears: Number(localStorage.getItem('ontop_horizon') || 5)
        };

        const response = await fetch(`${API_BASE_URL}/api/finance-advisor`, {
            method: 'POST',
        ...
        hideFinanceTyping();
        if (data.success) {
            addFinanceChatMessage(data.response, 'emma');
        } else {
            addFinanceChatMessage('Unable to generate advice right now. Please try again.', 'emma');
        }
    } catch (err) {
        console.error('Finance advisor error', err);
        hideFinanceTyping();
